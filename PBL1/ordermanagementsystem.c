...
Food createFood(int ID, char name[100], double price, int amount);
...
Food createFood(int ID, char name[100], double price, int amount) {
    ...
    food->amount = amount;
    food->price = price;
    food->total = (double)food->amount * food->price;
    return food;
}
...
void selectFood(Menu foodList, Menu order){
    ...
    do {
        ...
        if (choice <= foodList->count && choice > 0 && amount > 0){
			...
			if(amount <= 0){
				printf("%d is invalid amount!\n",amount);
			}
			else{
				printf("%d is invalid ID!\n", choice);
			} 
			printf("The order (%d - %d) is not accepted!\n", choice, amount);
		}        
...
void writeToFile(Menu mn) {
    ...
    for (int p = 0; p < mn->count; p++) {
        fprintf(file, " %d - %d : ", mn->list[p].ID, mn->list[p].amount);
    }
